...
export const handler: Handlers = {
  async POST(req, _ctx) {
    ...
    pubsub.publish({ item, method: "POST" });
    return new Response();
  },
  ...
  async PATCH(req, _ctx) {
    ...
    if (result.data.field === "done") {
      item = await Items.findAndModify(
        { _id },
        {
    ...
    const pubsub = PubSub.instance;
    pubsub.publish({ item, method: "PATCH" });
    return new Response();
  },

  async DELETE(req, _ctx) {
    const json = await req.json();
    const result = z.object({ _id: z.string() }).safeParse(json);
    if (!result.success) {
      return new Response(null, { status: 400 });
    }
    const _id = new ObjectId(result.data._id);
    ...
    const pubsub = PubSub.instance;
    pubsub.publish({ item, method: "DELETE" });
    return new Response();
  },
};
